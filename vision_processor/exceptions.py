...
class ModelInferenceError(VisionProcessorError):
    ...
    def __init__(self, model_name: str, image_path: Optional[Path] = None, 
...
class ImageProcessingError(VisionProcessorError):
    ...
    def __init__(self, image_path: Path, operation: str, 
                 original_error: Optional[Exception] = None, **kwargs):
...
class ExtractionError(VisionProcessorError):
    ...
    def __init__(self, message: str, extracted_text: Optional[str] = None, **kwargs):
        details = {}
        if extracted_text:
            details["extracted_text_preview"] = extracted_text[:200] + "..." if len(extracted_text) > 200 else extracted_text
        details.update(kwargs)
        super().__init__(message, details)
...
class MemoryError(VisionProcessorError):
    ...
    def __init__(self, required_mb: float, available_mb: float, 
                 operation: str = "model loading", **kwargs):
...
class BatchProcessingError(VisionProcessorError):
    ...
    def __init__(self, total_files: int, failed_files: int, 
                 failed_paths: Optional[list[Path]] = None, **kwargs):
